...
import { acknowledge, getFlags, getInbox, getQueue, markRead, removeFlag, saveFlag, submitDraft } from './api.js'
...
function Item({ item, children }) {
  return <li className="submission-item">
    ...
    {item.note && <p className="submission-note">{item.note}</p>}{children}
  </li>
}
  ...
  return <section className="submissions weekly-submission" aria-label="This week's submission">
    ...
    {!data ? <p role="status">{remote.error ? 'Could not load your draft. Use Refresh to try again.' : 'Loading your draft…'}</p> : <>
      ...
      {!sectors.length ? <p>No sector assigned. Ask an admin to update your coverage in the directory.</p> : <>
        ...
        <ul className="submission-items">{flags.map(item => <Item key={item.id} item={item}>{editable && <div className="submission-actions"><button disabled={busy} onClick={() => setEditor(item)}>Edit</button><button disabled={busy} onClick={() => act(() => removeFlag(item.id))}>Remove</button></div>}</Item>)}</ul>
...
function DigestCard({ submission, auth, onAck, busy, children }) {
...
function SectorQueue({ auth }) {
  ...
  return <><div className="submission-toolbar"><h2>Sector submissions</h2><button onClick={remote.reload}>Refresh</button></div><ErrorNotice error={error || remote.error} />
    {!data ? <p role="status">{remote.error ? 'Could not load submissions.' : 'Loading sector submissions…'}</p> : <>
      ...
      {sent.map(submission => <DigestCard key={submission.id} submission={submission} auth={auth} onAck={ack} busy={busy} />)}
  ...
  return <section className="submissions inbox-page">
    ...
    {tab === 'queue' ? <SectorQueue auth={auth} /> : <><ErrorNotice error={error || remote.error} />
      {!remote.data ? <p role="status">{remote.error ? 'Could not load your inbox. Use Refresh to try again.' : 'Loading inbox…'}</p> : !messages.length ? <div className="submission-empty"><h2>Your inbox is clear</h2><p>{isAnalyst(auth) ? 'When a sector lead acknowledges your weekly submission, it will appear here.' : 'Submitted digests for your assigned sectors will appear here.'}</p></div> : Object.entries(groups).map(([uid, group]) => <div key={uid} className="submission-message-group"><h2>{group[0].analyst}</h2>{group.map(message => <div className={message.read_at ? '' : 'submission-unread'} key={message.message_id}>
        ...
        <DigestCard submission={message} auth={auth} busy={busy} onAck={id => act(() => acknowledge(id))}>{!message.read_at && <button disabled={busy} onClick={() => act(() => markRead(message.message_id))}>Mark as read</button>}</DigestCard>
      </div>)}</div>)}
    </>}
  </section>
}
